...
@Service
public class JsonMetadataParser implements DataFileMetadataParser {
    @Resource
    ...
    static {
        ...
        defaultOptions.put("primitivesAsString", "true");
    }

    @Override
    public Boolean support(DataFileType type) {
        return type == DataFileType.JSON;
    }
    ...
    public DataFileMetadata parse(DataFile dataFile) {
        ...
        Dataset<Row> ds = sparkService.getSparkSession().read()
                .options(options)
                .json("s3a://" + minioConfig.getBucketName() + "/" + objectName);
