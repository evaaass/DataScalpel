...
/**
 * {@code DmpOracleTemplates} is an SQL dialect for Oracle
 *
 * <p>tested with Oracle 10g XE</p>
 *
 * @author tiwe
 */
public class DsOracleTemplates extends DsSQLTemplates implements Serializable {

    @SuppressWarnings("FieldNameHidesFieldInSuperclass") //Intentional
    public static final DsOracleTemplates DEFAULT = new DsOracleTemplates(true);
    private static final long serialVersionUID = -546922154702173413L;

    public static Builder builder() {
        return new Builder() {
            @Override
            protected SQLTemplates build(char escape, boolean quote) {
                return new DsOracleTemplates(escape, quote);
            }
        };
    }

    private String outerQueryStart = "select * from (\n select a.*, rownum \"_SYS_RN_\" from (\n  ";
    ...
    private String limitOffsetTemplate = "\"_SYS_RN_\" > {0s} and rownum <= {1s}";

    private String offsetTemplate = "_SYS_RN_ > {0}";

    private String bulkInsertTemplate = "insert all";

    private String bulkInsertSeparator = " into ";

    public DsOracleTemplates() {
        this('\\', false);
    }

    public DsOracleTemplates(boolean quote) {
        this('\\',quote);
    }

    public DsOracleTemplates(char escape, boolean quote) {
